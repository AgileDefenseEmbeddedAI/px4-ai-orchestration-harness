...
from harness.validator import engine as validator_engine
...
async def _plan_mission(
    ...
    try:
        ...
        verdict = validator_engine.run(mig, constraints_config)
        is_valid = verdict["result"] == "accept"
        errors = [v["description"] for v in verdict.get("violations", [])]
        ...
        missions[mission_id].verdict = verdict
...
@router.get("/plan/{mission_id}/verdict", summary="Get validator verdict for a plan")
async def get_verdict(mission_id: str, request: Request):
    """Return the stored verdict artifact for a mission."""
    missions: dict = request.app.state.missions
    if mission_id not in missions:
        raise HTTPException(status_code=404, detail=f"Mission '{mission_id}' not found")
    mission = missions[mission_id]
    if mission.verdict is None:
        raise HTTPException(
            status_code=404,
            detail=f"No verdict available yet for mission '{mission_id}' (status: {mission.status})"
        )
